...
//		result.addAll(getSubtitlesSubUnacs(ve));
	...
	private void parseEntrySubSab(VideoEntry ve, List<SubtitleArchiveEntry> subtitleList, Document doc) {
		for (Element link : doc.getElementsByTag("a")) {
			...
			if (matcher.matches()) {
				...
				try {
					subtitleZip = downloadSubtitleToTemp(SITE.SUBS_SAB, ve.getAcceptableFileName(), href);
	...
	private void parseEntrySubUnacs(VideoEntry ve, List<SubtitleArchiveEntry> subtitleList, Document doc) {
		for (Element link : doc.getElementsByTag("a")) {
			String href = link.attr("href");
			Matcher matcher = pattern_unacs_subLinks.matcher(href);
			if (matcher.matches()) {
				href = "https://subsunacs.net" + href;

				// JSoup gets the wrong thing sometimes
				// String subName = link.textNodes().get(0).toString();
				String subName = ve.getFileName();

				Path subtitleZip = null;
				try {
					subtitleZip = downloadSubtitleToTemp(SITE.SUBUNACS, ve.getAcceptableFileName(), href);
				} catch (ParseException | IOException e) {
					e.printStackTrace();
				}
				if (subtitleZip != null) {
